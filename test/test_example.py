...
def test_fail():
    result = 1 + 1
    assert result != 3 # forkert, 1 + 1 er 2 ikke 3

def test_another_fail():
    assert "hello".lower() != "HELLO" # forkert, skal være "hello"

@pytest.mark.focus(reason="This test is expected to fail.") # forventet fejl
...
@pytest.mark.skip(reason="This test is expected to fail.") # testen bliver skippet
